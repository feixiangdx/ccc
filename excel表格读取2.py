...
wd = xlrd.open_workbook(filename=r"D:\Python学习\day08\2020年每个月的销售情况.xlsx")  # 打开Excel表格
# 全年的销售总额
sales_sum = []
sumx = 0
for s in range(0, 12):
    sheet = wd.sheet_by_index(s)
...
cloth_sales_volume = {}  # 创建字典
cloth_sum = 0
for s in range(0, 12):
    sheet = wd.sheet_by_index(s)
    for i in range(1, sheet.nrows):  # 获取行进行遍历
        row_value = sheet.row_values(i)  # 获取值
        cloth_sum += row_value[4]
        if row_value[1] not in cloth_sales_volume.keys():
            cloth_sales_volume[row_value[1]] = row_value[4]  # 不存在就新建key
        else:
            cloth_sales_volume[row_value[1]] += row_value[4]  # 存在就相加
print("每件衣服的销售（件数）占比如下：")
for c in cloth_sales_volume.keys():  # 将字典中的键进行调用
    print("{}:{}%".format(c, round((cloth_sales_volume[c] / cloth_sum) * 100, 2)))  # format将c和round分别给了前面
print("--------------------------------------------------")
